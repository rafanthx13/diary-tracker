...
function TotalsCard({ title, totals }: { title: string; totals: TimeTotal[] }) {
  return <section className="rounded-3xl border border-stone-200 bg-white p-5 sm:p-6"><h2 className="text-xl font-semibold">{title}</h2>{totals.length ? <div className="mt-4 divide-y divide-stone-100">{totals.map((item) => <div key={item.label} className="flex items-center justify-between gap-4 py-3 first:pt-0"><span className="min-w-0 truncate font-medium">{item.label}</span><span className="shrink-0 tabular-nums text-sm font-semibold text-emerald-800">{formatReportDuration(item.minutes)}</span></div>)}</div> : <p className="mt-4 rounded-2xl bg-stone-50 px-4 py-5 text-center text-sm text-stone-500">Sem atividades encerradas neste período.</p>}</section>;
}
