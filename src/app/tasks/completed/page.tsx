...
function CompletedTaskCard({ task }: { task: Task }) { return <article className="flex min-h-44 flex-col justify-between rounded-3xl border border-stone-200 bg-white p-5 opacity-80 shadow-sm"><div className="flex items-start justify-between gap-4"><h3 className="text-lg font-semibold leading-6 text-stone-600 line-through">{task.title}</h3><div className="flex shrink-0 flex-col gap-2"><form action={setTaskCompletion}><input type="hidden" name="taskId" value={task.id} /><input type="hidden" name="complete" value="false" /><button aria-label={`Reabrir ${task.title}`} title="Reabrir tarefa" className="grid size-9 place-items-center rounded-xl bg-emerald-700 text-lg text-white transition hover:bg-emerald-800">✓</button></form><EditTaskModal task={task} /><DeleteTaskButton task={task} /></div></div><dl className="mt-6 space-y-1 text-sm text-stone-500"><div>Criada: {formatDateTime(task.created_at)}</div><div>Concluída: {task.completed_at ? formatDateTime(task.completed_at) : "—"}</div></dl></article>; }
