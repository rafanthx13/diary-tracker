...
import { DeleteTaskButton, EditTaskModal } from "./task-modals";
...
function TaskCard({ task }: { task: Task }) {
  return <article className={`flex min-h-40 flex-col rounded-3xl border p-5 shadow-sm ${task.is_important ? "border-amber-300 bg-amber-50" : "border-stone-200 bg-white"}`}><div className="flex items-start justify-between gap-3"><h3 className="text-lg font-semibold leading-6">{task.title}</h3><div className="flex shrink-0 flex-col gap-2"><form action={setTaskImportance}><input type="hidden" name="taskId" value={task.id} /><input type="hidden" name="important" value={task.is_important ? "false" : "true"} /><button aria-label={`${task.is_important ? "Remover importância de" : "Marcar como importante"} ${task.title}`} aria-pressed={task.is_important} title={task.is_important ? "Remover importância" : "Marcar como importante"} className={`grid size-9 place-items-center rounded-xl border-2 text-lg font-black transition ${task.is_important ? "border-amber-500 bg-amber-500 text-white" : "border-stone-300 text-stone-400 hover:border-amber-500 hover:text-amber-600"}`}>!</button></form><form action={setTaskCompletion}><input type="hidden" name="taskId" value={task.id} /><input type="hidden" name="complete" value="true" /><button aria-label={`Concluir ${task.title}`} title="Concluir tarefa" className="grid size-9 place-items-center rounded-xl border-2 border-stone-300 text-lg transition hover:border-emerald-600 hover:bg-emerald-50">✓</button></form><EditTaskModal task={task} /><DeleteTaskButton task={task} /></div></div>{task.is_important && <p className="mt-auto pt-6 text-sm font-medium text-amber-800">! Importante</p>}</article>;
}
